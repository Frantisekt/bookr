...
class TestLoggedInGreetingView(TestCase):
    ...
    def test_user_greeting_not_authenticated(self):
        ...
        request.user = AnonymousUser()  # Assigning the AnonymousUser() instance to the property makes the view function think that the user making the request is not logged in.
        response = greeting_view_user(request)
        self.assertEquals(response.status_code, 302)

    def test_user_authenticated(self):
        request = self.client.get('/test/greet_user')
